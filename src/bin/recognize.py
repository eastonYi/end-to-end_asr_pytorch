...
def fast_recognize(args):
    from data import AudioDataLoader, AudioDataset

    cv_dataset = AudioDataset(args.recog_json, args.batch_size,
    ...
    for i, data in enumerate(cv_loader):
        ...
        pred, gold = model.recognize(padded_input, input_lengths, padded_target)

        if i % args.print_freq == 0:
            print('Iter {} | {:.1f} ms/batch'.
                  format(i + 1, 1000 * (time.time() - start) / (i + 1)),
                  flush=True)
