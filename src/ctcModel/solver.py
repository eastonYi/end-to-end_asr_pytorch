...
from ctcModel.loss import cal_performance
...
class Solver(object):
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):
        ...
        for i, data in enumerate(data_loader):
            ...
            logits = self.model(padded_input, input_lengths)
            ctc_loss = cal_performance(logits, padded_target, smoothing=self.label_smoothing)
            loss = ctc_loss
            if not cross_valid:
            ...
            if i % self.print_freq == 0:
                print('Epoch {} | Iter {} | Current Loss {:.3f} | Learning Rate {:.3e} | {:.1f} ms/batch | step {}'.
                      format(epoch + 1, i + 1, ctc_loss.item(), self.optimizer.optimizer.param_groups[0]["lr"],
